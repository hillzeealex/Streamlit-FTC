...
def manual_mood_page():
    ...
    for i, (mood, value) in enumerate(mood_manual_mapping.items()):
        ...
        with column:
            if st.button(mood, key=f"manual_mood_{value}"):
                st.session_state["manual_mood"] = value
...
def results_page():
    ...
    payload = {
        ...
        "manual_mood": st.session_state.get("manual_mood", None)
    ...
    try:
        ...
        model_results = {key: data[key] for key in ["result1", "result2", "result3"] if key in data}
